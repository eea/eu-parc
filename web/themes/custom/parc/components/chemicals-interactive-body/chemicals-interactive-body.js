    ...
    Drupal.behaviors.chemicalsInteractiveBody = {
        attach(context) {
            ...
            elements.forEach((el) => {
                ...
                const chemicals = [
                    "Arsenic", "Pesticides", "Phthalates", "PFASs", "Bisphenol",
                    "Mercury",
                ];
                const TARGET_FPS = 60;
                ...
                const initCentroids = (layerSvg, id) => {
                    ...
                    centroids.forEach(centroid => {
                        ...
                        if (label) {
                            ...
                            const colors = [...new Set(allPills.map(getButtonColor))];
                            centroidColors.set(key, colors);
                        }
                    });
                    layerIdToCentroids.set(id, centroids);
                };
                    ...
                    labels.forEach(lbl => {
                        ...
                        pills.forEach(pill => {
                            pill.addEventListener('click', () => {
                                const parts = pill.id.split('_');
                                if (parts.length >= 2) {
                                    handlePillClick(parts[1].toLowerCase());
                                }
                            });
                        });
                    });
